...
import { getAthletes, isLoggedIn } from "../../api/client"
    ...
    <div className="rounded-2xl overflow-hidden relative"
      ...
      <div className="p-4">
        <div className="flex items-start justify-between gap-3">
          ...
          <button onClick={() => onFollow(athlete._id)}
            className="flex-shrink-0 flex items-center gap-1.5 rounded-xl px-3 py-1.5 text-xs font-bold transition-all"
            style={{ background:followed?"transparent":ACCENT, color:followed?ACCENT:"#fff", border:`1px solid ${ACCENT}` }}>
      ...
      <div className="p-3">
        <div className="flex items-start justify-between gap-2 mb-2">
          ...
          <button onClick={() => onFollow(athlete._id)}
            ...
            style={{ background:followed?"transparent":ACCENT, color:followed?ACCENT:"#fff", borderColor:ACCENT }}>
            {followed ? "✓" : "+"}
          </button>
        </div>
...
export default function AthleteExplore() {
  ...
  const handleFollow = id => setFollowed(prev => ({ ...prev, [id]: !prev[id] }))
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {featured.map((athlete, i) => (
                    <div key={athlete._id} className="sc-card-enter" style={{ animationDelay:`${i*80}ms` }}>
                      <FeaturedCard athlete={athlete} dark={dark} onFollow={handleFollow} followed={!!followed[athlete._id]} />
                    </div>
                  ))}
                </div>
              </div>
                  ...
                  {athletes.map((athlete, i) => (
                    <div key={athlete._id} className="sc-card-enter" style={{ animationDelay:`${i*50}ms` }}>
                      <AthleteCard athlete={athlete} dark={dark} onFollow={handleFollow}
                        followed={!!followed[athlete._id]} index={i} />
                    </div>
                  ))}
                </div>
              )}
