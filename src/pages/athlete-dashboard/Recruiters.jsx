...
import {
  ...
  getAthleteById, getOffers, startConversation, sendMessage, getRecruiters
} from "../../api/client"
...
export default function AthleteRecruiters() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        const { data } = await getRecruiters({ search: query.trim() })
        // Map API results to the same shape as recruiter cards
        const mapped = (data.recruiters || []).map(r => ({
          id: r.id,
          recruiterId: r.id,
          name: `${r.firstName} ${r.lastName}`,
          title: r.role || "Recruiter",
          school: r.organization || "—",
  ...
  const filtered = searchMode
    ? searchResults  // show search results when typing
    : recruiters.filter(r => {
      const matchFilter = filter === "all" ? true
        : filter === "unread" ? r.unread
              ...
              <div className="flex-1 min-w-0 space-y-3">
                {fetching ? (
                  ...
                  <div className="rounded-2xl py-10 text-center"
                    ...
                    <p className="text-sm" style={{ color: tk.textMuted }}>Searching recruiters...</p>
                  </div>
                ) : filtered.length === 0 ? (
                  <div className="rounded-2xl px-6 py-16 text-center"
                    ...
                    <p className="font-bold text-sm mb-1" style={{ color: tk.text }}>
                      {searchMode ? `No recruiters found for "${query}"` : "No recruiters yet"}
                    </p>
                    <p className="text-xs" style={{ color: tk.textMuted }}>
                      {searchMode
                        ? "Try searching by name or institution"
                        : filter !== "all"
