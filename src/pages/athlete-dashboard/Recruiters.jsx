...
const RECRUITERS_DATA = [
  { id:1, name:"Coach Daniel Owusu",    title:"Head Soccer Coach",    school:"University of Ghana",         location:"Accra, Ghana",      sport:"Soccer", division:"Division I",   verified:true,  interest:"high",   lastView:"2 hours ago", totalViews:8, message:"James, your defensive stats from the InterSchools tournament caught my eye. We'd love to discuss opportunities at UG.", unread:true,  saved:false },
  { id:2, name:"Coach Abena Kumi",      title:"Athletics Director",    school:"KNUST",                       location:"Kumasi, Ghana",     sport:"Soccer", division:"Division I",   verified:true,  interest:"high",   lastView:"Yesterday",   totalViews:5, message:"We've been following your highlights closely. Your positioning and reading of the game is exceptional for your age.",  unread:true,  saved:true  },
  { id:3, name:"Coach Marcus Webb",     title:"Assistant Coach",       school:"Ashesi University",           location:"Berekuso, Ghana",   sport:"Soccer", division:"Division II",  verified:false, interest:"medium", lastView:"3 days ago",  totalViews:3, message:"Hi James, we have a scholarship opening for a center back in the 2026 intake. Let's connect.",                        unread:false, saved:false },
  { id:4, name:"Coach Esi Mensah",      title:"Women's Soccer Coach",  school:"Central University",          location:"Accra, Ghana",      sport:"Soccer", division:"Division II",  verified:true,  interest:"medium", lastView:"5 days ago",  totalViews:2, message:"Your highlight reel from the Eastern Regional finals was very impressive. We'd like to learn more.",                 unread:false, saved:true  },
  { id:5, name:"Coach Kwesi Boateng",   title:"Sports Director",       school:"Accra Technical University",  location:"Accra, Ghana",      sport:"Soccer", division:"Division III", verified:false, interest:"low",    lastView:"1 week ago",  totalViews:1, message:"Viewed your profile while reviewing Eastern Region talent.",                                                             unread:false, saved:false },
  { id:6, name:"Coach Nana Sarpong",    title:"Head Coach",            school:"Ghana Institute of Mgmt",     location:"Accra, Ghana",      sport:"Soccer", division:"Division II",  verified:true,  interest:"new",    lastView:"Just now",    totalViews:1, message:"",                                                                                                                       unread:true,  saved:false },
]

const ACTIVITY = [
  { id:1, type:"view",    recruiter:"Coach Daniel Owusu", school:"University of Ghana",  time:"2h ago",     icon: Eye           },
  { id:2, type:"message", recruiter:"Coach Abena Kumi",   school:"KNUST",                time:"Yesterday",  icon: MessageCircle },
  { id:3, type:"save",    recruiter:"Coach Marcus Webb",  school:"Ashesi University",    time:"3 days ago", icon: Bookmark      },
  { id:4, type:"view",    recruiter:"Coach Nana Sarpong", school:"GIMS",                 time:"Just now",   icon: Eye           },
  { id:5, type:"view",    recruiter:"Coach Esi Mensah",   school:"Central University",   time:"5 days ago", icon: Eye           },
]
...
function InterestBadge({ level, dark }) {
  ...
  return (
    <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold flex-shrink-0"
      ...
      <span className="w-1.5 h-1.5 rounded-full inline-block" style={{ background: meta.dot, animation: meta.pulse ? "scPulse 2s infinite" : "none" }} />
      {meta.label}
    </span>
  )
}
      ...
      <div className="p-4">
        ...
        <div className="flex items-start gap-3">
          <div className="relative flex-shrink-0">
            ...
            {recruiter.verified && (
              <div className="absolute -bottom-0.5 -right-0.5 w-4 h-4 rounded-full flex items-center justify-center" style={{ background: ACCENT }}>
        ...
        <div className="flex items-center justify-between mt-3 pt-3" style={{ borderTop: `1px solid ${tk.border}` }}>
          <div className="flex items-center gap-3">
            <span className="text-xs flex items-center gap-1" style={{ color: tk.textMuted }}>
              <Eye className="w-3 h-3" />{recruiter.totalViews} view{recruiter.totalViews !== 1 ? "s" : ""}
            </span>
            <span className="text-xs flex items-center gap-1" style={{ color: tk.textMuted }}>
...
function ActivityItem({ item, dark }) {
  const tk = dark ? THEME.dark : THEME.light
  const Icon = item.icon
  const cfg = {
    view:    { label: "viewed your profile", color: ACCENT,    bg: "rgba(29,168,255,0.1)"  },
    message: { label: "sent you a message",  color: "#10B981", bg: "rgba(16,185,129,0.1)"  },
    save:    { label: "saved your profile",  color: "#F59E0B", bg: "rgba(245,158,11,0.1)"  },
  }[item.type]
  return (
    <div className="flex items-center gap-3 py-2.5" style={{ borderBottom: `1px solid ${tk.border}` }}>
      ...
      <div className="flex-1 min-w-0">
        <p className="text-xs" style={{ color: tk.textSub }}>
          <span className="font-bold" style={{ color: tk.text }}>{item.recruiter}</span> {cfg.label}
        </p>
        <p className="text-xs" style={{ color: tk.textMuted }}>{item.school}</p>
      </div>
      <span className="text-xs flex-shrink-0" style={{ color: tk.textMuted }}>{item.time}</span>
    </div>
  )
}

function ProfileStrength({ dark }) {
  const tk = dark ? THEME.dark : THEME.light
  const items = [
...
function MessageModal({ recruiter, dark, onClose }) {
  const tk = dark ? THEME.dark : THEME.light
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4"
      ...
      <div className="sc-modal w-full sm:max-w-md rounded-3xl overflow-hidden"
        ...
        <div className="px-5 py-4 flex items-center justify-between" style={{ borderBottom: `1px solid ${tk.border}` }}>
          ...
          <button onClick={onClose} className="w-8 h-8 rounded-xl flex items-center justify-center" style={{ color: tk.textMuted }}>
            <X className="w-4 h-4" />
          </button>
        </div>
        {recruiter.message && (
        ...
        <div className="px-5 pb-5">
          ...
          <div className="flex gap-2 mt-3">
            <button onClick={onClose}
              ...
              style={{ color: tk.textSub, borderColor: tk.border }}>Cancel</button>
            <button className="flex-1 py-2.5 rounded-xl text-sm font-bold text-white hover:opacity-90 transition-opacity"
              style={{ background: ACCENT }}>Send Reply</button>
          </div>
        </div>
      </div>
    </div>
...
export default function AthleteRecruiters() {
  ...
  const [recruiters, setRecruiters] = useState(RECRUITERS_DATA)
  ...
  const handleSave    = id => setRecruiters(prev => prev.map(r => r.id === id ? { ...r, saved: !r.saved } : r))
  ...
  return (
    <div className="min-h-screen transition-colors duration-300"
        ...
        <main className="flex-1 min-w-0 pb-28 lg:pb-12"
          ...
          <div className="sticky top-14 sm:top-16 z-30 backdrop-blur-xl px-4 pt-5 pb-4"
            ...
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-xs mt-0.5" style={{ color: tk.textMuted }}>
                  {unreadCount > 0
                    ? `${unreadCount} new ${unreadCount === 1 ? "message" : "messages"} from scouts`
            ...
            <div className="flex items-center gap-2 rounded-2xl px-3 py-2.5 mb-3"
              ...
              <input value={query} onChange={e => setQuery(e.target.value)}
                ...
                style={{ color: tk.text }} />
              {query && <button onClick={() => setQuery("")} style={{ color: tk.textMuted }}><X className="w-3.5 h-3.5" /></button>}
            </div>
          ...
          <div className="px-4 py-4">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
              <StatCard icon={Eye}      label="Profile Views"    value="47" accent dark={dark} />
              <StatCard icon={Users}    label="Total Recruiters" value={recruiters.length} dark={dark} />
              <StatCard icon={Flame}    label="High Interest"    value={recruiters.filter(r => r.interest === "high").length} dark={dark} />
              <StatCard icon={BookOpen} label="Messages"         value={recruiters.filter(r => r.message).length} dark={dark} />
            </div>

            {/* Two-col layout: list + sidebar */}
            <div className="flex gap-4 items-start">
              ...
              <div className="flex-1 min-w-0 space-y-3">
                {filtered.length === 0 ? (
                  <div className="rounded-2xl px-6 py-16 text-center"
                    ...
                    <p className="font-bold text-sm mb-1" style={{ color: tk.text }}>No recruiters found</p>
                    <p className="text-xs" style={{ color: tk.textMuted }}>
                      {filter !== "all" ? "Try switching to 'All' to see everyone" : "Keep building your profile to attract scouts"}
                    </p>
                  </div>
                ) : (
                  filtered.map((r, i) => (
              ...
              <div className="hidden lg:flex flex-col gap-4 w-64 flex-shrink-0">
                <ProfileStrength dark={dark} />

                {/* Activity feed */}
                <div className="rounded-2xl p-4" style={{ background: tk.surface, border: `1px solid ${tk.border}` }}>
                  <div className="flex items-center gap-2 mb-1">
                    <Bell className="w-4 h-4" style={{ color: ACCENT }} />
                    <h3 className="font-bold text-sm" style={{ color: tk.text }}>Recent Activity</h3>
                  </div>
                  <p className="text-xs mb-3" style={{ color: tk.textMuted }}>Last 7 days</p>
                  {ACTIVITY.map(item => <ActivityItem key={item.id} item={item} dark={dark} />)}
                </div>
                ...
                <div className="rounded-2xl p-4"
                  ...
                  <button className="mt-3 text-xs font-bold flex items-center gap-1" style={{ color: ACCENT }}>
                    View all tips <ArrowUpRight className="w-3 h-3" />
                  </button>
                </div>
              </div>
            </div>
      ...
      {/* Message modal */}
      {modal && <MessageModal recruiter={modal} dark={dark} onClose={() => setModal(null)} />}
    </div>
  )
}