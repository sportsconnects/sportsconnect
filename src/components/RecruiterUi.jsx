...
// ATHLETE ROW
export function AthleteRow({ athlete, dark, onSave, onView, saved: savedProp = false, border = true }) {
    ...
    <aside className="hidden lg:flex flex-col sticky top-16 h-[calc(100vh-4rem)] py-4 pr-2 w-52 xl:w-60 flex-shrink-0">
      ...
      <div className="mt-auto px-3 py-3 rounded-xl"
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="min-w-0">
            <p className="text-xs font-bold truncate" style={{ color: tk.text }}>{name}</p>
            {institution && (
              <p className="text-xs truncate" style={{ color: tk.textMuted }}>{institution}</p>
            )}
          </div>
        </div>
      </div>
